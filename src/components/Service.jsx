 ...
 useEffect(() => {
  ...
  return ( 
    <div className="shadow-sm p-4 my-4">
      ...
      <button type="button" key={service.id}    onClick={() => {
       handleShow()
       
     localStorage.setItem("service",service.id)
     console.log( localStorage.getItem("service"))
      }} className="btn btn-outline-info mx-2 my-3">{service.name}</button>
...
function PopupAdd( {show ,setShow}){
  ...
  return (
    <>
      ...
      >
        ...
        <Form.Group
            ...
            >
              ...
              <Form.Control as="textarea" rows={2}  name="discription"
        ...
        <Modal.Footer>
          ...
          <Button variant="primary"  type="submit" value="save" onClick={ 
      () => {
      ...
      // Handle validations
      axios
        .post("http://127.0.0.1:8000/api/create-service", service)
        .then(response => {
